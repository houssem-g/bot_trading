...
class check_trade(threading.Thread):
    ...
    def run(self):
        ...
        try:
            monnaie = I_want_money.get_async_order(self.id)["position-changed"]["msg"]["raw_event"]["instrument_underlying"]
        ...
        while True:
            try:
                ...
                if (now > 30):
                    print(" I am gonna stop 30 seconds")
                    time.sleep(30)
                else:
                    print(" I am gonna stop ", now, " seconds")
                    time.sleep(now)
...
class multi_trade(threading.Thread):
    ...
    def run(self):
        ...
        _,id = I_want_money.buy_digital_spot_v2(self.monnaie,price,self.action,duration)
        try:
            # time.sleep(2)
            PL= I_want_money.get_digital_payout(self.monnaie)
        except Exception as e:
            print(e)
    ...
    def loopPrediction(self):
        ...
        if len(resMonnaie)>0:
            ...
            if (len(list_id) > 0):
                ...
                # checkWin["nb"].iloc[0] > 1 or
                if (checkWin["nb"].iloc[0] > 3 or int(datetime.datetime.now().hour) > 15):
            ...
            checkWin = pd.read_sql_query('''
                    ...
                    ''', conn)
            print("number of trades : ", checkWin)
            # or (len(checkWin.index) > 0 and checkWin["nb"].iloc[0] > 1)
            if (int(datetime.datetime.now().hour) > 15 or (len(checkWin.index) > 0 and checkWin["nb"].iloc[0] > 3)):
                print("Fin de journée.")
                exit()
