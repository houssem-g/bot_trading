...
class get_data_multithread(threading.Thread):
    ...
    def run(self):
        ...
        data= I_want_money.get_candles(self.monnaie, 900, 4, end_from_time)
        for el in data:
...
class main():
    ...
    def getFiguresAndWriteToDb(self):
        ...
        currentSecondes = datetime.now().second
        if (int(currentDate) < 15):
            now = 900 - currentSecondes - (int(currentDate) * 60)
        elif ( 15 <= int(currentDate) < 30):
            now = 1800 - currentSecondes - (int(currentDate) * 60)
        elif ( 30 <= int(currentDate) < 45):
            now = 2700 - currentSecondes - (int(currentDate) * 60)
        elif ( 45 <= int(currentDate)):
            now = 3600 - currentSecondes - (int(currentDate) * 60)
        else: now = 900
